...
export default function CreateTitle({ token, genres = [], onCreated, editTitle = null }) {
	...
	// one ref per media slot + capsule + header
	...
	const [form, setForm] = useState(editTitle ? {
		name: editTitle.name,
	...
	function handleChange(e) { setForm((f) => ({ ...f, [e.target.id]: e.target.value })); }
	function setFile(field, file) { setFiles((f) => ({ ...f, [field]: file })); }
	...
	function goPrev() { setActiveSlot(ALL_MEDIA[(currentIndex - 1 + ALL_MEDIA.length) % ALL_MEDIA.length]); }
	function goNext() { setActiveSlot(ALL_MEDIA[(currentIndex + 1) % ALL_MEDIA.length]); }
	...
	async function handleSubmit(e) {
		...
		if (editTitle) {
			...
			const res = await fetch(`/api/title/${encoded}`, {
				method: "PATCH",
			...
			if (!res.ok) {
				setError(data.detail ?? "Error al actualizar el título");
				setLoading(false);
				setProgress(null);
				return;
			}
			...
			const res = await fetch("/api/title/", {
				method: "POST",
				headers: { "Content-Type": "application/json", Authorization: `Bearer ${token}` },
				body: JSON.stringify({ name: form.name, release_date: form.release_date, release_price: form.release_price, actual_discount: discount }),
			});
			const data = await res.json();
			if (res.status !== 201) {
		...
		if (mediaEntries.length > 0) {
			...
			const mediaRes = await fetch(`/api/title/${encoded}/media`, { method: "PATCH", headers: { Authorization: `Bearer ${token}` }, body: fd });
			if (!mediaRes.ok) {
				const mediaErr = await mediaRes.json().catch(() => ({}));
				setError(mediaErr.detail ?? "Error al subir los archivos");
				setLoading(false);
				setProgress(null);
				return;
			}
	...
	// ── hidden file inputs (outside visual tree) ──
	const hiddenInputs = (
		<div className="hidden">
			<input ref={headerRef} type="file" accept="image/*" onChange={(e) => setFile("header", e.target.files[0] ?? null)} />
			<input ref={capsuleRef} type="file" accept="image/*" onChange={(e) => setFile("capsule", e.target.files[0] ?? null)} />
			{STORE_FIELDS.map((f) => (
				<input key={f} ref={storeRefs[f]} type="file" accept="image/*" onChange={(e) => setFile(f, e.target.files[0] ?? null)} />
			))}
			<input ref={trailerRef} type="file" accept="video/mp4" onChange={(e) => setFile("trailer", e.target.files[0] ?? null)} />
		</div>
	);
	...
	return (
		<form onSubmit={handleSubmit}>
			...
			<div className="mb-6 overflow-hidden rounded-xl border border-burnt-border bg-burnt-card">
				...
				<button
					...
					className="group relative block w-full overflow-hidden bg-burnt-card aspect-460/215"
					title="Subir header"
				>
					{files.header ? (
					...
					<div className="mb-3 grid gap-3" style={{ gridTemplateColumns: "3fr 8fr" }}>
						...
						>
							{files.capsule ? (
								...
								<AuthImage src={existingUrl("capsule")} alt="Capsule" onError={() => markFailed("capsule")} />
							) : (
						...
						<div className="relative aspect-video w-full overflow-hidden rounded-md border border-burnt-border bg-burnt-surface">
							...
							{activeSlot === "trailer" ? (
								files.trailer ? (
									<video className="relative z-1 h-full w-full" src={fileUrl(files.trailer)} controls autoPlay muted />
								) : existingUrl("trailer") ? (
									<div className="relative z-1 h-full w-full">
										<video className="h-full w-full" src={existingUrl("trailer")} controls muted />
										<button
											type="button"
											onClick={(e) => { e.stopPropagation(); triggerUpload("trailer"); }}
								...
								<img src={fileUrl(files[activeSlot])} alt={activeSlot} className="relative z-1 h-full w-full object-cover" />
							) : existingUrl(activeSlot) ? (
								<div className="relative z-1 h-full w-full">
									<AuthImage src={existingUrl(activeSlot)} alt={activeSlot} onError={() => markFailed(activeSlot)} />
								</div>
							) : (
								<div className="pointer-events-none absolute inset-0 z-1 flex flex-col items-center justify-center gap-2 border-2 border-dashed border-burnt-border/40 text-burnt-faint">
					...
					<div className="flex flex-wrap justify-center gap-1.5">
						{STORE_FIELDS.map((field) => (
							<button
								...
								className={`h-14 w-24 flex-none overflow-hidden rounded border-2 transition-colors ${activeSlot === field ? "border-burnt-accent" : "border-burnt-border hover:border-burnt-border"}`}
							>
								{files[field] ? (
						...
						<button
							...
							className={`flex h-14 w-24 flex-none items-center justify-center gap-1.5 rounded border-2 text-xs font-medium transition-colors ${activeSlot === "trailer" ? "border-burnt-accent bg-burnt-accent/10 text-burnt-accent" : "border-burnt-border bg-burnt-surface text-burnt-muted hover:border-burnt-border"}`}
						>
		...
		<div className="space-y-6">
			...
			<div className="rounded-lg border border-burnt-border bg-burnt-card p-5">
					<p className="mb-4 text-xs font-semibold uppercase tracking-widest text-burnt-faint">Información básica</p>
					<div className="space-y-4">
						<div>
							<label className="mb-1.5 block text-sm text-burnt-muted" htmlFor="name">
								Nombre <span className="text-burnt-red">*</span>
							</label>
							<input id="name" type="text" value={form.name} onChange={handleChange} required placeholder="Título del juego"
								readOnly={!!editTitle}
						...
						<div>
							...
							<input id="release_date" type="date" value={form.release_date} onChange={handleChange} required
						...
						<div className="grid grid-cols-2 gap-3">
							<div>
								...
								<input id="release_price" type="number" min="0" step="0.01" value={form.release_price} onChange={handleChange} required placeholder="0.00"
							...
							<div>
								<label className="mb-1.5 block text-sm text-burnt-muted" htmlFor="actual_discount">Descuento (%)</label>
								<input id="actual_discount" type="number" min="0" max="100" value={form.actual_discount} onChange={handleChange}
									className="w-full rounded-lg border border-burnt-border bg-burnt-panel px-4 py-2.5 text-sm text-burnt-text focus:border-burnt-accent focus:outline-none" />
							</div>
						</div>
					</div>
				...
				<div className="space-y-4">
					{error && (
						<p className="rounded-lg border border-burnt-red/30 bg-burnt-red/10 px-4 py-2.5 text-sm text-burnt-red">{error}</p>
					)}
					...
					{!editTitle && (
						<p className="text-center text-xs text-burnt-faint">Quedará pendiente de aprobación del administrador.</p>
					)}
				</div>
			</div>
		</form>
